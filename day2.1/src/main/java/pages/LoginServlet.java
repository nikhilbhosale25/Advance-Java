	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // login failed
				...
				//create  a cookie to hold user dbtls
		...
		//		pw.flush();//sending the resp (commiting the resp)
				...
				if (user.isVotingStatus()) {// alrdy voted
					...
					//WC clears (i.e emtyis or discard)the resp buffer 
					//sends temp redirect resp : SC 302 | location=candidate_list 
					//set-cookie :
					//user-dtls: user.details.toString
					//| body : EMPTY
					//web browser : checks privacy settings --cokies blocked=>seession tracking fails
